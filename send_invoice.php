...
$mm = new Mail_mime("\n");
$text = "This is your invoice";
$mm->setTxtBody($text);
$mm->addAttachment($output, 'application/pdf', 'Invoice-' . $invoiceValues['order_id'] . '.pdf', false);
$companyName = $invoiceValues['Company'];
$subject = "$companyName's Invoice";
$body = $mm->get();
$companyName = $invoiceValues['Company'];
$headers = $mm->headers(array('From '.$companyName.'', 'Subject' => $subject));
$email = $invoiceValues['Email'];
$mail = Mail::factory('mail');
if($email ==""){
    echo '<script type="text/javascript">';
     echo 'alert("There is no email associated with this invoice");';
     echo 'window.location.href = "invoice_list.php";';
     echo '</script>';
}else{
...
$email_send = new sendmail();

try {
	$email_send->Email($companyName, $email, $output, $invoiceValues);
    //Server settings                    //Enable verbose debug outpu
	echo '<script type="text/javascript">';
	     echo 'alert("Email successfully sent");';
	     echo 'window.location.href = "invoice_list.php";';
	     echo '</script>';
