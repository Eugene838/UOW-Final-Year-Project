...
if (isset($_POST["login"])) {
	...
	$stmt = $db->connectWix()->prepare("SELECT * FROM wixPatients WHERE email = ? and NRIC_PNum = ?");
	$stmt->execute([$email, $password]);

	if ($stmt->rowCount() > 0) {
		$patient = $stmt->fetchAll(PDO::FETCH_ASSOC);
		$_SESSION['Patient_ID'] = $patient[0]["Patient_ID"];
		$_SESSION['Patient_PostalCode'] = $patient[0]["postal_code"];
...
<style>
	...
	body {
		margin-top: -100px;
		background-image: url('img/patient-bg.jpg');
...
<body>
	<div class="container text-center mb-5">
		<img id="top-bg" src="" alt="bgtop" width="300" height="300">
		<img id="bot-bg" src="" alt="bgbot" width="300" height="300">
		...
		<form class="" method="POST" style="max-width: 480px; margin:auto;">
			<h1 class="text-nowrap mt-5">Welcome to Dentian</h1>
